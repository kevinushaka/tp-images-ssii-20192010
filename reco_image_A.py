...
def reconnaissance_RegressionLogistique():
    ...
    if verbose:
        ...
        #if argv[5] == "True" :
            # print("F1Score :", f1_score(labelsValid, prediction))

    classif = svm.SVC(C=45)
    classif.fit(bows, labelsValid)
    predictionSVM = classif.predict(bows)
    print('prediction class for bows (SVM)', predictionSVM)
    print("F1Score (SVM): ", f1_score(labelsValid, predictionSVM, average='macro'))
    #print('support vectors: ', classif.support_vectors_)

    print("F1Score : ", f1_score(labelsValid, prediction, average='macro'))
...
#      TEST      #
##################
listImgTest = glob.glob("test/moto/*.jpg")
nbMotos = len(listImgTest) # Nombre d'élements dans la classe moto
labelsValid = [0]*nbMotos # On remplie label avec autant de 0 que de motos

listImgTest += glob.glob("test/voiture/*.jpg")
nbVoitures = len(listImgTest)-nbMotos # Nombre d'elements dans la classe voiture
labelsValid += [1]*nbVoitures # On remplie label avec autant de 1 que de voitures


##################
#   VALIDATION   #
##################
# listImgTest = glob.glob("valid/moto/*.jpg")
# nbMotos = len(listImgTest) # Nombre d'élements dans la classe moto
# labelsValid = [0]*nbMotos # On remplie label avec autant de 0 que de motos
#
# listImgTest += glob.glob("valid/voiture/*.jpg")
# nbVoitures = len(listImgTest)-nbMotos # Nombre d'elements dans la classe voiture
# labelsValid += [1]*nbVoitures # On remplie label avec autant de 1 que de voitures
